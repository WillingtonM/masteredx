...
if(isset($_POST)) {
  ...
  if (isset($_POST['login_username']) && isset($_POST['login_password'])) {
    ...
    if (!$data['error']) {
      ...
      if ($usr_res  = get_user_by_email($username)) {
        $found_user = try_login($username, $password, 'email');
        ...
        if ($found_user) {
          ...
          $_SESSION['email_confirm'] = ($usr_res['email_confirm']) ? TRUE : FALSE;
          $data['success']  = true;
        ...
        $data['error']      = true;
        $data['message']    = "You currently don't have an account with us";
      }
    }
  }
  ...
  if (isset($_POST['form_type']) && $_POST['form_type'] == 'member_add') {
    ...
    $surname_init   = (isset($_POST['surname_initials'])) ? $_POST['surname_initials'] : '';

    $member_id      = (isset($_POST['post_user'])) ? $_POST['post_user'] : '';

    $data['error']    = (empty($surname_init)) ? true : false;
    $data['message']  = (empty($surname_init)) ? "Surname Intials input field is empty" : '';

    if (!$data['error'] && empty($member_id)) {
      if (get_member_by_member_surname_initials($surname_init)) {
        $data['error']    = false;
    ...
    if (!$data['error']) {
      ...
      if (!empty($member_id)) {
        ...
        $msg_dta['old'] = array(
          ...
          'reference'         => $member['member_reference'],
          'surname_initials'  => $member['member_surname_initials'],
        );

        $key_dta  = 'update';
        $ins_sql  = "UPDATE members SET member_name = ?, member_surname = ?, member_surname_initials = ?, member_location = ?, member_reference = ? WHERE member_id = ? LIMIT 1";
        $ins_dta  = [$name, $surname, $surname_init, $location, $reference, $member_id];
      } else {
        $ins_sql  = "INSERT INTO members (company_id, office_id, member_name, member_surname, member_surname_initials, member_location, member_reference, user_id) VALUES (?,?,?,?,?,?,?,?)";
        $ins_dta  = [$company_id, $office_id, $name, $surname, $surname_init, $location, $reference, $user_id];
        $key_dta  = 'insert';
        $msg_dta  = 'insert';
      }
      ...
      if (prep_exec($ins_sql, $ins_dta, $sql_request_data[2])) {
        $notif_db   = 'members';
        $member_id  = ($msg_dta == 'insert') ? $connect->lastInsertId() : $member_id;
        $msg_dta    = (is_array($msg_dta)) ? json_encode($msg_dta, true) : $msg_dta;
        if (insert_notifications($user_id, $member_id, $notif_db, $member_id, 0, 0, $msg_dta, $key_dta)) {
          $data['url']      = 'refresh';
          $data['success']  = true;
        }
      }
  ...
  if (isset($_POST['form_type']) && $_POST['form_type'] == 'member_update') {
    ...
    if (!empty($member_id) && $member) {
      ...
      $member_name        = $member['member_surname_initials'];
      $msg_dta            = array();
      $new_sql_dta        = array();
      $old_sql_dta        = array();
      ...
      foreach ($practice_tasks as $key => $val) {
        ...
        if (isset($_POST['activity'.$key]) && !empty($_POST['activity'.$key])) {
          ...
          // update activity
          if ($activity_task  = get_activity_tasks_by_practice_task ($member_id, $task_id)) {
            update_activity_task ($member_date, $user_id, $member_id, $task_id);
          } else {
            insert_activity_task ($user_id, $member_id, $task_id, $member_date);
          }
          
          // update and send email
          if ($upd_sts == false) {
            insert_notifications($user_id, $member_id, $notif_db, $task_id, 0, 0, $member_date, $update_type);
            ...
            if (isset($val['practice_task_sms']) && $val['practice_task_sms'] && (is_array($clients) || is_object($clients)) ) {
              foreach ($clients as $keys => $value) {
                ...
                if ((!$err || $err == '200') && isset($response)) {
                  ...
                  $ins_sql    = "INSERT INTO sms_orders (user_id, member_id, practice_task_id, order_confirmation, confirmation_token, order_complete, order_amount, order_amount_net, order_payment_id, order_token, order_status, order_payment_status, order_message, order_billing_date) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
                  $ins_dta    = [$usr_id, $member_id, $task_id, 1, '', 1, $amount, $amount, $payment_id, '', 1, 1, $msg_res, $date];
  ...
  // remove member
  if (isset($_POST['form_type']) && $_POST['form_type'] == 'remove_estate') {
    $usr_id = (isset($_POST['user'])) ? $_POST['user'] : '';

    if (get_member_by_member_id($usr_id)) {
      $stmnt_sql = "UPDATE members SET member_status = 0 WHERE member_id = ? LIMIT 1";
      $stmnt_dta = [$usr_id];
      ...
      if (prep_exec($stmnt_sql, $stmnt_dta, $sql_request_data[2])) {
        ...
        $notif_db         = 'members';
        $state_msg        = 'remove';
        if (insert_notifications($user_id, $usr_id, $notif_db, $usr_id, 0, 0, $state_msg, 'member_status')) {
        }
  ...
  if (isset($_POST['form_type']) && $_POST['form_type'] == 'remove_member') {
    ...
    if (!empty($member_id)) {
      $rmv_sql = "UPDATE associations set association_status = 0, creator_user_id = ? WHERE user_id = ? AND member_id = ? LIMIT 1";
      $rmv_dta = [$user_id, $usr_id, $member_id];

      if (prep_exec($rmv_sql, $rmv_dta, $sql_request_data[2])) {

        $notif_db         = 'associations';
        $state_msg        = 'update';
        if (insert_notifications($user_id, $member_id, $notif_db, $usr_id, 0, 0, 0, 'association_status')) {
        }
  ...
  if (isset($_POST['form_type']) && $_POST['form_type'] == 'search_assign') {
    ...
    $user             = get_member_by_user_id($usr_id, $member_id, false);
    ...
    if (!$data['error']) {
      if (!$user) {
        $rmv_sql  = "INSERT INTO associations (creator_user_id, practice_area_id, user_id, member_id) VALUES (?, ?, ?, ?)";
      } elseif ($user && $user['association_status'] != 1) {
        $rmv_sql  = "UPDATE associations set association_status = 1, creator_user_id = ?, practice_area_id = ? WHERE user_id = ? AND member_id = ?  LIMIT 1";
      }

      $rmv_dta    = [$user_id, $practice_area_id, $usr_id, $member_id];

      if (prep_exec($rmv_sql, $rmv_dta, $sql_request_data[2])) {

        $notif_db = 'associations';
        $db_table = ($user) ? 'association_status' : '';
        if (insert_notifications($user_id, $member_id, $notif_db, $usr_id, 0, 0, $state_msg, $db_table)) {
        }
  ...
  // deseased estate data initite processing
  if (isset($_POST['form_type']) && $_POST['form_type'] == 'dasesed_estate_data') {
    $practice_area_id        = (isset($_POST['practice_area'])) ? $_POST['practice_area'] : '';
    ...
    if (is_uploaded_file($_FILES['data_file']['tmp_name']) === false) {
      ...
      if (move_uploaded_file($image['tmp_name'], $img_url)) {
        ...
        foreach ($excel_data as $row) {
          ...
          // if (empty($col_1) || $col_1 == null) continue;

          // // member variables
          $member = get_member_by_surname_init($col_1);

          // // user variables
          // // $col_0              = (int) $col_0;
          $col_3              = (!empty($col_3) && !is_null($col_3)) ? preg_replace("/\s+/", "", $col_3) : NULL;
          $col_3              = (!empty($col_3)) ? str_replace(' ', '', $col_3) : NULL;

          $name               = (is_null($col_2) || empty($col_2)) ? null : $col_2;
          $phone              = ( (is_null($col_3) || empty($col_3)) && !empty($name) ) ? $name : $col_3;
          $email              = (is_null($col_4) || empty($col_4)) ? null : $col_4;
          ...
          if ( ($member == null || empty($member)) && !empty($col_1) ) {
            ...
            $client_sql       = "INSERT INTO members (user_id, company_id, office_id, member_location, member_reference, member_surname_initials) VALUES (?,?,?,?,?,?)";
            $client_dta       = [$user_id, $company_id, $office_id, $location, $col_0, $col_1];
          ...
          if (!empty($phone) && isset($last_member_id) && !empty($last_member_id)) {
            $assoc_sql        = "INSERT INTO associations (user_id, member_id, creator_user_id, practice_area_id) VALUES (?, ?, ?, ?)";
  ...
  if (isset($_POST['form_type']) && $_POST['form_type'] == 'data_download') {
    ...
    if (!$data['error']) {
      ...
      $sql        = "SELECT * FROM members m INNER JOIN  associations a ON m.member_id = a.member_id INNER JOIN users u ON u.user_id = a.user_id WHERE m.member_status = 1 AND a.practice_area_id = ?";
      ...
      if (is_array($wb_data) || is_object($wb_data)) {
        ...
        foreach ($wb_data as $key => $member) {
          ...
          foreach ($practice_task as $task)
          {
            $activity_task  = get_activity_tasks_by_practice_task($member['member_id'], $task['practice_task_id']);
