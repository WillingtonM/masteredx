        ...
        <div class="col-12">
                            ...
                            <tbody>
                                ...
                                <?php foreach ($req_res as $key => $association) : ?>
                                    ...
                                    <tr>
                                        ...
                                        <td class="">
                                            <a class="float-end me-3" type="butto" onclick="requestModal(post_modal[10], post_modal[10], {'user_id':<?= $association['user_id'] ?>, 'user_type':'guest'})"> <i class="fa-solid fa-pen-to-square me-1"></i> Edit </a>
            ...
            <?php else : ?>
                <div class="row shadow-sm p-3 mb-5 bg-white rounded" style="border-radius: 15px !important;">
                    ...
                    if (HOST_IS_LIVE) {
                        $cnt_sql        = "SELECT * FROM registry WHERE  registry_status = 1 " . $user_sql;
                        $cnt_dta        = [];
                    ...
                    if (HOST_IS_LIVE) {
                        $rgst_sql       = "SELECT * FROM registry WHERE  registry_status = 1 " . $user_sql . " ORDER BY registry_date_created DESC LIMIT $sql_pg_strt, $intval";
                        $rgst_dta       = [];
                                ...
                                <?php if (is_array($nwsf_qry) || is_object($nwsf_qry)) : ?>
                                    ...
                                    <?php foreach ($nwsf_qry as $key => $association) : ?>
                                        ...
                                        <tr>
                                            ...
                                            <td class="">
                                                <?php if ($is_admin && !$apr) : ?>
                                                    <a class="float-end me-3 px-3 cursor-pointer border-radius-lg btn-secondary text-sm" type="butto" onclick="requestModal(post_modal[17], post_modal[17], {'user':<?= $association['registry_user_id'] ?>, 'registry': <?= $association['registry_id'] ?>})"> <i class="fa-solid fa-pencil me-1"></i> Edit </a>
                                                <?php elseif (!$apr) : ?>
                                                    <a class="float-end me-3 px-3 cursor-pointer border-radius-lg btn-secondary text-sm" type="butto" onclick="postCheck('null', {'form_type':'registry_approve', 'user':<?= $association['registry_user_id'] ?>, 'registry': <?= $association['registry_id'] ?>})"> <i class="fa-solid fa-thumbs-up me-1"></i> Approve </a>
                                                <?php else : ?>
                                                    <Span class="float-end me-3 px-3/ cursor-pointer/ border-radius-lg/ btn-secondary/ text-sm"> No action </Span>
                                                <?php endif; ?>
