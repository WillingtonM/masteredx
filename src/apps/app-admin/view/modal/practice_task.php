        ...
        <div class="col-12" id="form_member">
            ...
            <form id="practiceAreaForm" class="form-horizontal">
                ...
                <div class="col-12">
                    ...
                    <button type="button" class="btn shadow-none text-danger float-end border-radiusl-lg" onclick="postCheck('member_err', {'form_type':'practice_task_remove', 'practice_id':<?= $practice['practice_task_id'] ?>}, 0); return;"> <i class="fa fa-trash me-2" aria-hidden="true"></i> Remove</button>
                    <?php endif; ?>
                    <button type="button" class="btn btn-sm btn-secondary px-3 shadow-none <?= (isset($practice_id) && !empty($practice_id)) ? '' : 'col-12' ?>" style="border-radius: 11px;" onclick="function_tinytext_forms_modal('practiceAreaForm', 'textarea_header', 'member_err')" <?= ((!$is_admin) ? '' : '') ?>> <?= (($practice != null) ? 'Edit' : 'Add') ?> Task </button>
                </div>
