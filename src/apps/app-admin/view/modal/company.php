            ...
            <form id="companyForm" class="form-horizontal">
                ...
                <?php else: ?>
                    ...
                    <div class="col-12">
                        ...
                        <?php endif; ?>
                        <button type="button" class="btn btn-sm btn-secondary px-3 shadow-none <?= (isset($office_id) && !empty($office_id)) ? '' : 'col-12' ?>" style="border-radius: 11px;" onclick="postCheck('office_err', $('#companyForm').serialize(), 0, true)" <?= ((!$is_admin) ? '' : '') ?>> <?= (($practice != null) ? 'Edit' : 'Add') ?> Office </button>
                    </div>
