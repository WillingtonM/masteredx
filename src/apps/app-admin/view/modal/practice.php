        ...
        <div class="col-12" id="form_member">
            ...
            <form id="practiceForm" class="form-horizontal" action="" method="POST">
                ...
                <div class="col-12">
                    ...
                    <button type="button" class="btn shadow-none text-danger float-end border-radiusl-lg" onclick="postCheck('member_err', {'form_type':'practice_remove', 'practice_id':<?= $practice['practice_area_id'] ?>}, 0); return;"> <i class="fa fa-trash me-2" aria-hidden="true"></i> Remove</button>
                    <?php endif; ?>
                    <button type="button" class="btn btn-secondary shadow-none btn-sm border-radiusl-lg <?= (isset($practice_id) && !empty($practice_id)) ? '' : 'col-12' ?>" onclick="postCheck('member_err', $('#practiceForm').serialize(), 0, true);" <?= ((!$is_admin) ? '' : '') ?>> <?= (($practice != null) ? 'Edit' : 'Add') ?> </button>
                </div>
