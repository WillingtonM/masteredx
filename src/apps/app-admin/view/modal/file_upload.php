...
<div class="row">
    ...
    <div class="col-12">
        <?php if (isset($media_res) && !empty($media_res)) : ?>
            <a type="button" class="btn btn-sm border-radius-lg text-danger float-end border border-3 border-danger" onclick="postCheck('error_pop', {'media_remove':true,'media_id':<?= $media_res['media_id'] ?>});"> <i class="fa fa-trash" aria-hidden="true"></i> &nbsp; Remove</a>
        <?php endif; ?>
        <button class="btn btn-warning btn-sm border-radius-lg" onclick="media_post()"><i class="fa fa-file-upload me-2"></i> <?= ((isset($_POST['media_id']) && $_POST['media_id'] != '') ? 'Edit' : 'Add') ?> Document</button>
    </div>
</div>
