    ...
    <div class="row">
        <div class="col-12 mt-3">
            <button class="btn border-radius-lg btn-warning me-3" onclick="modal_user_post()" <?= ((!$is_admin) ? 'disabled_' : '') ?>> <?= (($usr_arr != null) ? 'Edit' : 'Add') ?> User</button>
            <?php if (!empty($usr_arr)): ?>
                <button type="button" class="btn btn-danger" style="border-radius: 12px; font-weight: bolder" onclick="postCheck( 'users_message', {'user_id' : parseInt(<?= (isset($usr_arr['user_id'])) ? $usr_arr['user_id'] : 0 ?>), 'edit_user': 'edit'})" <?= ((!$is_admin) ? 'disabled_' : '') ?>> Remove user </button>
            <?php endif; ?>
        </div>
    </div>
