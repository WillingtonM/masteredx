...
if (isset($_GET['slgid']) && isset($_GET['type']) ) {
  ...
  $cmnt_sql = "SELECT * FROM article_comments WHERE article_id = ? AND article_comment_type = 0 AND article_comment_status = 1 ORDER BY article_comment_date_created LIMIT 150";
  $cmnt_dta = [$article_id];
  ...
  // article db date
  $artcl_date     = ($req_res) ? DateTime::createFromFormat('Y-m-d H:i:s', $req_res['article_publish_date']) : '';
  $artcl_dateud   = ($req_res) ? DateTime::createFromFormat('Y-m-d H:i:s', $req_res['article_date_updated']) : '';
  $article_link   = ($req_res) ? urlencode(host_url('/article?article=' . $slugify->slugify($req_res["article_title"]) . '&slgid=' . $req_res["article_id"] . '&type=' . $req_res["article_type"])) : '';
}
