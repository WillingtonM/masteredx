    ...
    if (!$data['error']) {
      if( $usr_res  = get_user_by_username($username) ) {
        ...
        if($found_user) {
          ...
          $_SESSION['company_id']   = (isset($found_user['company_id']) && !empty($found_user['company_id'])) ? $found_user['company_id'] : '';
          $_SESSION['office_id']    = (isset($found_user['office_id']) && !empty($found_user['office_id'])) ? $found_user['office_id'] : '';
          $_SESSION['user_type']    = (isset($found_user['user_type_id']) && !empty($found_user['user_type_id'])) ? $found_user['user_type_id'] : '';
    ...
    if (!$data['error']) {
      ...
      if (prep_exec($events_sql, $events_dta, $sql_request_data[2])) {
        ...
        $client_ifo = array(
          ...
          "event_country"     => $countries_array[$event_country],
          "message_type"      => 'public',
