...
function getPath(){
  ...
  if (HOST_IS_LOCAL) {
    if ($url_count == 3) {
      ...
      $url_get    = "home";
      redirect_to(DS . PROJECT_NAME . DS . 'home');
    }
  } elseif ($url_count == 3) {
    $url_get    = substr($url, strpos($url, "/") + 1);
    $url_get    = ($url_get == '') ? 'home' : $url_get;
  } else {
    if ($url_count == 2) {
  ...
  if (isset($_SESSION['user_id']) && ($page === 'login' || $page === 'passreset')) {
    redirect_to('home');
...
{
  global $deceased_estates;

  $reversed_ds = array_reverse($deceased_estates, true);
  ...
  if ($res_key) {
    ...
    $res_arr['key_stag']  = $deceased_estates[$res_key];
