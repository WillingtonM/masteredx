...
<div class="card-body p-3">
    ...
    <div class="col-12 py-3">
        ...
        <button type="button" class="btn btn-secondary btn-sm/ col-12" style="border-radius: 12px; font-weight: bolder" onclick="postCheck('<?= $bookng_msg ?>', $('#<?= $b_form_usr ?>').serialize() + '&' + $('#<?= $b_form_msg ?>').serialize() + '&' + $('.<?= $b_form_pne ?>.tab-pane.active').serialize(), 0, true);"> <?= (isset($_SESSION['user_id']) ? 'Submit Booking' : 'Request Quotation') ?> </button>
    
        <?php if (isset($_SESSION['user_id'])) : ?>
            <a type="button" class="btn btn-sm/ btn-warning shadow-none article_nav article_active float-end/" onclick="requestModal(post_modal[18], post_modal[18], {'type':'<?= $key ?>', 'event_id':<?= $event_id ?>})"> <i class="fa-solid fa-calendar-check me-2"></i> <span> <?= ($order) ? 'Edit': 'Register' ?> Quotation </span> </a>
        <?php endif ?>
    </div>
