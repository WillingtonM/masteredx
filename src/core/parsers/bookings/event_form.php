...
<div class="card-body p-3">
    ...
    <button type="button" class="btn btn-secondary me-3" style="border-radius: 12px; font-weight: bolder" onclick="postCheck('<?= $bookng_msg ?>', $('#<?= $b_form_usr ?>').serialize() + '&' + $('#<?= $b_form_msg ?>').serialize() + '&' + $('.<?= $b_form_pne ?>.tab-pane.active').serialize(), 0, true);"> <?= (isset($_SESSION['user_id']) ? 'Submit Event' : 'Submit Event') ?> </button>
    <?php if(isset($event_id) && !empty($event_id)): ?>
        <button type="button" class="btn btn-danger" style="border-radius: 12px; font-weight: bolder" onclick="postCheck('<?= $bookng_msg ?>', {'form_type':'event_remove','event':<?= $event_id ?>}, 0);"> Remove Event </button>
    <?php endif; ?>
