<div class="row shadow-sm px-0 mb-3 bg-white rounded" style="border-radius: 15px !important;">
    ...
    if (HOST_IS_LIVE) {
        $cnt_sql        = "SELECT * FROM notifications WHERE user_id = ?  AND notification_status = 1";
        $cnt_dta        = [$usr_id];
                    ...
                    <?php if (is_array($nwsf_qry) || is_object($nwsf_qry)) : ?>
                        ...
                        <?php foreach ($nwsf_qry as $key => $association) : ?>
                            ...
                            <tr>
                                ...
                                <td class="px-3">
                                    <a class="float-end me-3 cursor-pointer" type="button" onclick="requestModal(post_modal[15], post_modal[15], {'usr':<?= $association['notification_id'] ?>, 'msg':'<?= $notif_msg ?>' })"> <i class="fa-solid fa-eye me-1"></i> View </a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    <?php endif; ?>
