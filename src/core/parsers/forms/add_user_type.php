    ...
    <div class="col-12">
        ...
        <button class="btn shadow-none text-danger float-end border-radiusl-lg" onclick="postCheck('user_error_pop', {'form_type':'user_type_remove', 'user_type_id':<?= $req_res['user_type_id'] ?>}, 0);"> <i class="fa fa-trash me-2" aria-hidden="true"></i> Remove</button>
        <?php endif; ?>
		<button class="btn btn-secondary shadow-none btn-sm border-radiusl-lg <?= (isset($user_type_id) && !empty($user_type_id)) ? '' : 'col-12' ?>" onclick="postCheck('user_error_pop', $('#user_type_form').serialize(), 0, true);"> <?= ((isset($_POST['user_type_uid']) && !empty($_POST['user_type_uid'])) ? 'Edit' : 'Add') ?> User Type</button>
    </div>
