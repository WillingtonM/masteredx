                    ...
                    <?php if (is_array($usrt_qry) || is_object($usrt_qry)) : ?>
                        ...
                        <?php foreach ($usrt_qry as $key => $usr) : ?>
                            ...
                            <tr>
                                ...
                                <td class="">
                                    <?php if (!$is_default): ?>
                                        <a type="button" class="btn btn-secondary float-end btn-sm/ me-2" name="button" onclick="requestModal(post_modal[19], post_modal[19], {'user_type_uid': <?= $usr['user_type_id'] ?>})" style="padding: 5px 15px;" <?= ((!$is_admin) ? 'disabled' : '') ?>> <span><i class="fas fa-user-edit me-2"></i> Edit </span> </a>
                                    <?php endif; ?>
                                </td>
                            </tr>
                            <?php $count++ ?>
